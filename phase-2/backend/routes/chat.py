...
class ChatResponse:
    def __init__(self, message: str, tool_calls: list = None, conversation_id: int = 0):
        self.message = message
...
class MessageItem:
    def __init__(self, id: int, role: str, content: str, tool_calls: list = None, created_at: datetime = None):
        self.id = id
        self.role = role
...
class ChatHistoryResponse:
    def __init__(self, messages: List[MessageItem], conversation_id: int, total_count: int):
...
async def send_message(
    ...
    if not openai_key:
        ...
        assistant_content = (
            ...
            "AI agent not configured - please set OPENAI_API_KEY in backend .env "
            "to enable AI responses. For now, I'm just echoing your message back!"
        )
        tool_calls = []
    else:
        # Try to use AI agent if configured
        try:
            ...
            assistant_content = (
                f"I received: '{request.message}'. "
                "Sorry, AI is temporarily unavailable. Your message has been saved!"
            )
            tool_calls = []
